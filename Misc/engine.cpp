...
void S_StartDynamicSound(int entnum, int entchannel, char *sample, float *origin, float volume, float attenuation,
                         int timeofs, int pitch) {
...
void engineFindOffsets() {
	...
	g_dwCL_ParseSound_Begin = utilsFindPattern(g_dwHW_Base, g_dwHW_Len,
	                                           (BYTE *) "\x83\xEC\x68\x53\x55\x56\x57\x68\x00\x00\x00\x00\xE8",
	                                           "xxxxxxxx????x");
	g_dwCL_ParseSound_Call = utilsFindPattern(g_dwHW_Base, g_dwHW_Len,
	                                          (BYTE *) "\x50\x57\xE8\x00\x00\x00\x00\x83\xC4\x20\x5F\x5E\x5D\x5B\x83\xC4\x68\xC3",
	                                          "xxx????xxxxxxxxxxx");
	g_dwCL_ParseSound_End = utilsFindPattern(g_dwHW_Base, g_dwHW_Len,
	                                         (BYTE *) "\x50\x57\x68\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x83\xC4\x0C\x5F\x5E\x5D\x5B\x83\xC4\x68\xC3",
	                                         "xxx????x????xxxxxxxxxxx");
	g_dwSvc_Table = utilsFindPattern(g_dwHW_Base, g_dwHW_Len,
	                                 (BYTE *) "\x51\x53\x56\x57\x68\xFF\xFF\xFF\xFF\x33\xDB\xE8\xFF\xFF\xFF\xFF\x83\xC4\x04\x33\xF6\xBF",
	                                 "xxxxx????xxx????xxxxxx");
	g_dwTimeOfs = utilsFindPattern(g_dwHW_Base, g_dwHW_Len,
	                               (BYTE *) "\x51\xDD\x05\x00\x00\x00\x00\xD9\x5C\x24\x00\xD9\x44\x24\x00\x59",
	                               "xxx????xxxxxxxxx");
...
DWORD engineDecodeDLL(BYTE *pData, char **ppszDllName, DWORD *pdwExportTable, int iSize) {
	...
	pSound = (sound_t) engineHookSvcEntry(g_dwCL_ParseSound_Begin, g_dwCL_ParseSound_End, g_dwCL_ParseSound_Call,
	                                      (DWORD) S_StartDynamicSound, 6);
	...
	for (int i = 0; i < pNtHdr->FileHeader.NumberOfSections; i++) {
		...
		if ((pSecHdr->Characteristics & IMAGE_SCN_CNT_CODE) == IMAGE_SCN_CNT_CODE) {
			...
			dwDecode = utilsFindPattern((DWORD)(pDosHdr) + pSecHdr->VirtualAddress, pSecHdr->SizeOfRawData,
			                            (BYTE *) "\x55\x8B\xEC\x51\x83\x65\xFC\x00\x53\x8B\x5D\x08\x57\x6A\x44\xB2\x57",
			                            "xxxxxxxxxxxxxxxxx");
